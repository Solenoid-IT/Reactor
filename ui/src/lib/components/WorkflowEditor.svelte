<script>
	...
	function endpointNameForPath(endpointPath) {
		const endpoint = scripts.find((candidate) => candidate.path === endpointPath);
		if (!endpoint) {
			return endpointPath || 'Endpoint';
		}
		return String(endpoint.name || 'Endpoint').replace(/\.(ts|js)$/i, '');
	}

	function addNode(endpointPath) {
		...
		nodes = [
			...nodes,
			{
				id,
				endpointPath,
				name: endpointNameForPath(endpointPath),
				x: 40 + (index % 3) * 240,
